...
```{r}
table("stop"=inhib$stop,"krit"=inhib$krit)
```
...
```{r}
describe(errorRates) %>% as.data.frame() %>% select(.,mean,sd,median,min,max) %>% knitr::kable(.)
```
...
```{r}
inhib_crit_we_stop <- filter(inhib_crit_we, lag_stop==1)
```

```{r}
id <- inhib_crit_we_stop  %>% group_by(.,feed.f) %>%
              dplyr::summarize(., mean    = mean(rt),
                                  se      = sd(rt)/sqrt(length(rt)))


a <- inhib_crit_we_stop  %>% group_by(.,feed.f,vp) %>%
             dplyr::summarize(., mean    = mean(rt),
                                  se      = sd(rt)/sqrt(length(rt)))
...
id <- inhib_crit_we_stop  %>% group_by(.,feed.f,block.f) %>%
              dplyr::summarize(., mean    = mean(rt),
                                  se      = sd(rt)/sqrt(length(rt)))


a <- inhib_crit_we_stop  %>% group_by(.,feed.f,vp,block.f) %>%
             dplyr::summarize(., mean    = mean(rt),
                                  se      = sd(rt)/sqrt(length(rt)))
...
```{r}
t.test(rt ~ feed.f,data=inhib_crit_we_stop)

t.test(rt ~ feed.f,data=inhib_crit_we_stop,subset=block==4)
```
